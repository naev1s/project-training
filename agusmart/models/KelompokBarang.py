...
class KelompokBarang(models.Model):
    ...
    barang_ids = fields.One2many(comodel_name='agusmart.barang',
    ...
    def _compute_jml_item(self):
        for record in self:
            a = self.env['agusmart.barang'].search([('kelompokbarang_id', '=', record.id)]).mapped('name')
            b = len(a)
            record.jml_item = b
            record.daftar = a
